			...
			<form>
				<table class="table table-bordered .table-responsive text-center">
					...
					@foreach($products as $product)
						<tr>
							<td><img width="200px" class="img-responsive" src="{{ asset('storage/avatar/'.$product->attributes->img) }}"></td>
							<td>{{ $product->name }}</td>
							<td>
								<div class="form-group">
									<input class="form-control" min="1" type="number" name="quantity" value="{{ $product->quantity }}" onchange="updateCart(this.value, '{{ $product->id }}')">
								</div>
							</td>
							<td><span class="price">{{ number_format($product->price,0,',','.' ) }} đ</span></td>
							<td><span class="price">{{ number_format($product->price * $product->quantity,0,',','.' ) }} đ</span></td>
							<td><a href="{{ asset('cart/delete/' . $product->id) }}">Xóa</a></td>
						</tr>
					@endforeach
				</table>
